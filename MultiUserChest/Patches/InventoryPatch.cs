...
namespace MultiUserChest.Patches {
    ...
    public static class InventoryPatch {
        ...
        public static bool AddItem1Prefix(Inventory __instance, ItemDrop.ItemData item, int amount, int x, int y, ref bool __result) {
            bool intercepted = InterceptAddItem(__instance, item, amount, new Vector2i(x, y));
        ...
        public static bool AddItem2Prefix(Inventory __instance, ItemDrop.ItemData item, ref bool __result) {
            bool intercepted = InterceptAddItem(__instance, item, item.m_stack, new Vector2i(-1, -1));
        ...
        private static bool InterceptAddItem(Inventory inventory, ItemDrop.ItemData item, int amount, Vector2i pos) {
            ...
            if (from.ZNetView.IsOwner() && !to.ZNetView.IsOwner()) {
                if (to is ContainerInventoryOwner toContainer) {
                    toContainer.Container.AddItemToChest(item, from.Inventory, pos, from.ZNetView.GetZDO().m_uid, amount, true);
                    return true;
                }
            }
            ...
            if (!from.ZNetView.IsOwner() && to.ZNetView.IsOwner()) {
                if (from is ContainerInventoryOwner fromContainer) {
                    fromContainer.Container.RemoveItemFromChest(item, to.Inventory, pos, to.ZNetView.GetZDO().m_uid, amount, to.Inventory.GetItemAt(pos.x, pos.y));
                    return true;
                }
            }
