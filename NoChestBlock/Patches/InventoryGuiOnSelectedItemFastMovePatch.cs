    ...
    public class InventoryGuiOnSelectedItemFastMovePatch {
        ...
        private static bool FastMoveItem(InventoryGrid grid, ItemDrop.ItemData item, Vector2i fromPos) {
            ...
            if (grid.GetInventory() == gui.m_currentContainer.GetInventory()) {
                ...
                gui.m_currentContainer.RemoveItemFromChest(item, player, new Vector2i(-1, -1), item.m_stack, null);
            } else {
                gui.m_currentContainer.AddItemToChest(item, grid.GetInventory(), new Vector2i(-1, -1), player.GetZDOID(), item.m_stack, false);
            }
